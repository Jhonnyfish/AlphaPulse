...
func (h *ReportsHandler) GenerateDailyReportAuto() {
	...
	now := time.Now()
	var lines []string
	lines = append(lines, fmt.Sprintf("# 📊 AlphaPulse Daily Report — %s", now.Format("2006年01月02日")))
	lines = append(lines, "")
	lines = append(lines, fmt.Sprintf("> Generated: %s  ", now.Format("2006-01-02 15:04:05")))
	lines = append(lines, fmt.Sprintf("> Stocks analyzed: %d", len(results)))
	lines = append(lines, "")
	...
	for i, r := range results {
		...
		lines = append(lines, fmt.Sprintf("| %d | %s | %s | %.2f | %s %.2f%% | %.1f | %s |", i+1, r.name, r.code, r.price, arrow, r.chg, r.score, r.sig))
	}
	lines = append(lines, "")
	if len(results) > 0 {
		top := results[0]
