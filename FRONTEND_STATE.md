...
- [x] 3.2d WatchlistPage — 批量操作（多选删除、批量添加标签）
- [x] 3.3a CandidatesPage — 分层展示（核心/关注/观察三个区域）
- [x] 3.3b CandidatesPage — 点击个股弹出分析详情 Modal（不跳转）
- [x] 3.3c CandidatesPage — 雷达图展示各维度得分（动量/趋势/资金/技术/估值/情绪）
...
## 变更日志
- 2026-04-27 00:20 | 3.3c CandidatesPage 六维评分雷达图 | 新增RadarScores接口(6维度:动量/趋势/资金/技术/估值/情绪), generateRadarScores确定性mock, scoreToRadarColor红→绿渐变, 260px ECharts radar(渐变填充+背景带), StockDetailModal双栏布局(雷达+评分条) | files: CandidatesPage.tsx, StockDetailModal.tsx
- 2026-04-27 00:00 | 3.2d WatchlistPage 批量操作 | 编辑模式按钮(CheckSquare)、批量操作栏(已选N项+全选+批量删除+批量添加标签)、Checkbox选择(桌面表格+移动卡片)、删除确认Modal(glass morphism)、批量标签输入Modal(逗号分隔+预览)、选中行高亮accent/10、Promise.allSettled批量移除、退出编辑模式 | files: WatchlistPage.tsx
